...
export class OrderController {
  ...
  @Get("/:id")
  @HttpCode(200)
  async getOrderByIdController(@Headers("authorization") header: string, @Param("id") orderId: string): Promise<Order> {
  ...
  @Delete("/:id")
  @HttpCode(200)
  async deleteOrderController(@Headers("authorization") header: string, @Param("id") orderId: string) {
    const tokenSplit = header.split(" ")[1];
    if (!tokenSplit) {
      throw new UnauthorizedException("Not found token");
    }
